...
/**
 * thread reset functions
 * @return  Pointer to thread object, or NULL on failure.
 *
 */
int thread_reset(thread_t *t) 
{
    unsigned int flags = 0;
    ...
    if (flags & THREAD_FLAG_DEBUG_STACK_BOUNDS_CHECK) {
        memset(t->stack + THREAD_STACK_PADDING_SIZE, STACK_DEBUG_BYTE,
               t->stack_size - THREAD_STACK_PADDING_SIZE);
    } else {
        memset(t->stack, STACK_DEBUG_BYTE, t->stack_size);
    }
#endif
...
void remove_task_from_rqueue(thread_t *thread)
{
    if (!list_is_empty(&thread->queue_node))
...
/* support thread exit/sched during vcpu event handler  */
void thread_vcpu_sched(int type)
{
	switch (type) {
		case INT_RESCHEDULE:
			thread_preempt();
