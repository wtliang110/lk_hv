...
static void test_init(void)
{
	...
	case 3:
		...
		printf("gic version is 3 s %p c %p\n", gic_isenabler, gic_icenabler);
		break;
	}

	install_irq_handler(EL1H_IRQ, irq_handler);
	printf("enable local irq\n");
	local_irq_enable();
}
...
static void print_timer_info(void)
{
	...
	printf("CNTV_TVAL_EL0: 0x%016x\n", vtimer_info.read_tval());
}
