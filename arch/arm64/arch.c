...
static void arm64_cpu_early_init(void) {
    ...
    unsigned long hcr_val;
    /* modify by peter wang : adapt to el2  */
    /* HCR register to trap ints to el2  */
    hcr_val = ARM64_READ_SYSREG(HCR_EL2);
    hcr_val = hcr_val | HCR_INT_TRAP;
    ARM64_WRITE_SYSREG(HCR_EL2, hcr_val); 
