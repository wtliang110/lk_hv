...
static void gic_trigger_interrupt(int vector) 
{
	int reg =  vector / 32;
    	uint32_t mask = 1ULL << (vector % 32);

	printf("will set interrupt %d\n", vector);
        GICREG(0, GICD_ISPENDR(reg)) = mask;
}

#include <lib/console.h>
void inter_enable(int argc, const console_cmd_args *argv)
{
	int vector =  argv[1].i;
	int enable =  argv[2].i;
	printf("vector %d %d\n", vector, enable);
...
void inter_triger(int argc, const console_cmd_args *argv)
{
	int vector =  argv[1].i;
#if 0	
